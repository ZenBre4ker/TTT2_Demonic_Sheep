...
function ENT:Initialize()
	...
	self.CollideCount = 0
	self.Drop = false
	self:SetModel( "models/weapons/ent_ttt_supersheep.mdl" )
	self:SetHealth(100)
	self.Boost = false
	...
	if(SERVER) then
		...
		hook.Add("EntityTakeDamage", "Supersheep_BiggerHitbox" .. tostring(self), function(target, dmg)
...
if (IsValid(self) && IsValid(self.CorrespondingWeapon)) then
	...
	--self.CorrespondingWeapon:StopSound("supersheep_wind")
end
...
if self.Minify then
	if not self.Minified then
		...
		self.MinifiedStart = CurTime()
		self.Minified = false
		self.Owner:SetNWBool("supersheep_small", false)
...
function ENT:Remove()
	hook.Remove("EntityTakeDamage", "Supersheep_BiggerHitbox" .. tostring(self))
end
